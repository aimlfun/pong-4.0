...
namespace Pong.AI
{
    ...
    {
        /// <summary>
        /// How many layers of neurons (3+). Do not use 1.
...
#pragma warning restore CS8618
        ...
        /// LeakyReLU alpha.
        /// </summary>
        private const double alpha = 0.001f;
        ...
        /// <returns></returns>
        private static double PReLUActivationFunction(double input)
        {
            return input > 0 ? input : alpha * input;
        }

        /// <summary>
        /// Derivative (for back-propagation of PReLU).
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        private static double DerivativeOfPReLUActivationFunction(double input)
        {
        ...
        /// <returns></returns>
        internal unsafe double[] FeedForward(double[] inputs)
        {
            ...
            for (int layer = 1; layer < Layers.Length; layer++)
            {
                ...
                for (int neuronIndexForLayer = 0; neuronIndexForLayer < Layers[layer]; neuronIndexForLayer++)
                {
                    ...
                    for (int neuronIndexInPreviousLayer = 0; neuronIndexInPreviousLayer < Layers[layerminus1]; neuronIndexInPreviousLayer++)
                    {
                        // remember: the "weight" amplifies or reduces, so we take the output of the prior neuron and "amplify/reduce" it's output here
                        value += Weights[layerminus1][neuronIndexForLayer][neuronIndexInPreviousLayer] * Neurons[layerminus1][neuronIndexInPreviousLayer];
                    }
                    ...
                    Neurons[layer][neuronIndexForLayer] = TanHActivationFunction(value + Biases[layerminus1][neuronIndexForLayer]);
                }
            }
        ...
        /// <param name="inputs"></param>
        /// <param name="expected"></param>
        public unsafe void BackPropagateff(double[] inputs, double[] expected)
        {
            double[] output = FeedForward(inputs); // runs feed forward to ensure neurons are populated correctly

            double[][] gamma;

            int layersLengthMinus1 = Layers.Length - 1;
            int layersLengthMinus2 = Layers.Length - 2;
            ...
            for (int i = 0; i < output.Length; i++)
            {
                gamma[layersLengthMinus1][i] = (output[i] - expected[i]) * DerivativeOfTanHActivationFunction(output[i]); // Gamma calculation
            }

            for (int i = 0; i < Layers[^1]; i++) // calculates the w' and b' for the last layer in the network
            {
                Biases[layersLengthMinus2][i] -= gamma[layersLengthMinus1][i] * learningRate;

                for (int j = 0; j < Layers[^2]; j++)
                {
                    Weights[layersLengthMinus2][i][j] -= gamma[layersLengthMinus1][i] * Neurons[layersLengthMinus2][j] * learningRate; //*learning 
                }
            }

            for (int i = layersLengthMinus2; i > 0; i--) // runs on all hidden layers
            {
                int iminus1 = i - 1;
                int iplus1 = i + 1;
                ...
                for (int j = 0; j < Layers[i]; j++) // outputs
                {
                    ...
                    for (int k = 0; k < gamma[iplus1].Length; k++)
                    {
                        gamma[i][j] += gamma[iplus1][k] * Weights[i][k][j];
                    }
                ...
                for (int j = 0; j < Layers[i]; j++) // iterate over outputs of layer
                {
                    double gammaijlearningrate = gamma[i][j] * learningRate;

                    Biases[iminus1][j] -= gammaijlearningrate; // modify biases of network

                    for (int k = 0; k < Layers[iminus1]; k++) // iterate over inputs to layer
                    {
                        Weights[iminus1][j][k] -= gammaijlearningrate * Neurons[iminus1][k]; // modify weights of network
                    }
                }
            }
        }
        ...
        /// <param name="inputs"></param>
        /// <param name="expected"></param>
        public  void BackPropagate2(double[] inputs, double[] expected)
        {
            double[] output = FeedForward(inputs); // runs feed forward to ensure neurons are populated correctly
            ...
            for (int i = 0; i < Layers[^1]; i++) // calculates the w' and b' for the last layer in the network
            {
                double gammaLayersLengthMinus1i = gamma[layersLengthMinus1][i] * learningRate;

                Biases[layersLengthMinus2][i] -= gammaLayersLengthMinus1i;

                for (int j = 0; j < Layers[^2]; j++)
                {
                    Weights[layersLengthMinus2][i][j] -= gammaLayersLengthMinus1i * Neurons[layersLengthMinus2][j];
                }
            }
            ...
            for (int i = layersLengthMinus2; i > 0; i--) // runs on all hidden layers
            {
                ...
                for (int j = 0; j < Layers[i]; j++) // outputs
                {
                    ...
                    for (int k = 0; k < gamma[i + 1].Length; k++)
                    {
                        gammaij += gamma[i + 1][k] * Weights[i][k][j];
                    }

                    gamma[i][j] = gammaij * DerivativeOfTanHActivationFunction(Neurons[i][j]); //calculate gamma
                }
                ...
                for (int j = 0; j < Layers[i]; j++) // iterate over outputs of layer
                {
                    double gammaij = gamma[i][j] * learningRate;

                    Biases[iMinus1][j] -= gammaij; // modify biases of network

                    for (int k = 0; k < Layers[iMinus1]; k++) // iterate over inputs to layer
                    {
                        Weights[iMinus1][j][k] -= gammaij * Neurons[iMinus1][k]; // modify weights of network
                    }
                }
            }
        }
